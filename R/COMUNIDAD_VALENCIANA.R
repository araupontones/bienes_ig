...
#definir tablas --------------------------------------------------------------
#190
tables = paste("Table", seq(190,250,1))
...
valencia = datos_clean %>%
  filter(comunidad == "COMUNIDAD VALENCIANA") %>%
  filter(!is.na(orden))
...
export(valencia, file.path("intermediate_data", exfile))
